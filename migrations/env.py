...
def run_migrations_online() -> None:
    ...
    if context.get_x_argument(as_dictionary=True).get("tenant"):
        current_tenant = context.get_x_argument(as_dictionary=True).get("tenant")

    print(' '.join(filter(None, [">>>>>CT: ", current_tenant])))
    ...
    with connectable.connect() as connection:
        ...
        context.configure(
            connection=connection, target_metadata=target_metadata, version_table_schema=current_tenant,
        )

        with context.begin_transaction():
            if current_tenant:
                context.execute(text(f"CREATE SCHEMA IF NOT EXISTS {current_tenant}"))
            context.run_migrations()
